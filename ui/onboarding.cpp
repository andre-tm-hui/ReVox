...
Onboarding::Onboarding(SoundboardMenu *soundboardMenu, FXMenu *fxMenu,
                       QToolButton *fxMenuButton, MainInterface *mi,
                       QWidget *parent)
    : QWidget(parent),
      ui(new Ui::Onboarding),
      LoggableObject("Qt-Onboarding"),
  ...
  for (int i = 1; i <= totalSlides; i++) {
    ...
    slide->findChild<QGroupBox *>("instruction_" + n)
        ->setStyleSheet(
  ...
  connect(soundboardMenu, &SoundboardMenu::itemSelected, this,
          [=]() { next(); });
  connect(soundboardMenu, &SoundboardMenu::itemSelected, this,
          [=]() { mi->SetBlocked(false); });
  connect(soundboardMenu, &SoundboardMenu::clipRecorded, this, [=]() {
    if (currentSlide == 7) next();
  });
  connect(fxMenuButton, &QToolButton::clicked, this, [=]() { next(); });
  connect(fxMenu, &FXMenu::itemSelected, this, [=]() {
    if (currentSlide <= 20) next();
  });
...
void Onboarding::next() {
  ...
  if (currentSlide <= slides.size()) {
    showSlide();
    log(INFO, "Tutorial page " + std::to_string(currentSlide));
  } else {
    delete this;
  }
}
