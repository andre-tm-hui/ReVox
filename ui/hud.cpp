...
HUD::HUD(QWidget *parent)
    : QDialog(parent), LoggableObject("Qt-HUD"), ui(new Ui::HUD) {
  ui->setupUi(this);
...
void HUD::SetPosition(int pos) {
  ...
  if (pos >= 0 && pos < 8) {
    ...
    log(INFO, "HUD moved to position " + std::to_string(pos));
  } else {
    this->hide();
    log(INFO, "HUD disabled");
  }
}
...
void HUD::AddActiveEffect(QString fx) {
  if (activeFX.find(fx) == activeFX.end()) {
    ...
    ss << "background: rgba(0,0,0,100); border: 1px solid transparent; "
          "border-radius: "
       << itemHeight / 5
       << "px; color: rgba(255,255,255,100); margin: 5%; font-size: "
       << (int)(itemHeight / 2.6) << "px;";
    ...
    switch (position) {
      ...
      case 0:  // top right/left
        this->resize(this->width(), fxWidgets.size() == 0
                                        ? itemHeight
      ...
      case 1:
      case 6:  // top/bottom
        this->resize(fxWidgets.size() == 0 ? label->width()
...
void HUD::RemoveActiveEffect(QString fx) {
  if (activeFX.find(fx) != activeFX.end()) {
    ...
    switch (position) {
      ...
      case 2:
        for (unsigned i = 0; i < fxWidgets.size(); i++) {
          if (index != -1)
            fxWidgets[i]->move(fxWidgets[i]->pos().x(),
                               fxWidgets[i]->pos().y() - itemHeight);
      ...
      case 6:
        for (unsigned i = 0; i < fxWidgets.size(); i++) {
          if (index != -1)
            fxWidgets[i]->move(fxWidgets[i]->pos().x() - widget->width(),
                               fxWidgets[i]->pos().y());
          if (fxWidgets[i] == widget) index = i;
        }
    ...
    delete widget;
    log(INFO, fx.toStdString() + " removed");
  }
}
