...
class BindableButton : public QPushButton
{
    Q_OBJECT
public:
    BindableButton(std::shared_ptr<BaseManager> bm, std::string path, bool isNew = false, QWidget *parent = nullptr);
...
private:
    static void waitForRebind(BindableButton *hkb, std::shared_ptr<BaseManager> bm);
    void updateSelf();
