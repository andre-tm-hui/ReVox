...
HotkeyListItemWidget::HotkeyListItemWidget(std::shared_ptr<BaseManager> bm,
                                           ...
                                           bool isNew,
                                           QWidget *parent)
    : QWidget{parent}, bm(bm), path(path)
{
    ...
    icon->setStyleSheet("QWidget {image: url(:/icons/edit.png); background: transparent; padding: 6px;}");
    arrange(icon, 0, 1);*/

    label = new QLineEdit(this);
    connect(label, SIGNAL(textChanged(QString)), this, SLOT(labelChanged(QString)));
    label->setStyleSheet("QLineEdit {color: rgba(255,255,255,80); border-radius: 3px; background-color: #303030; margin: 3px;}"
...
void HotkeyListItemWidget::labelChanged(QString text) {
    if (text.size() > maxTextSize)
        label->setText(text.first(maxTextSize));
    if (text.size() == 0)
        label->setStyleSheet("QLineEdit {color: #606060; border-radius: 3px; background-color: #303030; margin: 3px;}"
                             "QLineEdit:hover {background-color: #404040;}");
    else
        label->setStyleSheet("QLineEdit {color: #D0D0D0; border-radius: 3px; background-color: #303030; margin: 3px;}"
                             "QLineEdit:hover {background-color: #404040;}");
