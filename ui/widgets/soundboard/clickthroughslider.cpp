...
void ClickthroughSlider::setDirection(bool leftToRight) {
  ...
  if (leftToRight) {
    ...
    this->setStyleSheet(
        "QSlider {background: transparent; position: absolute; left: -4px; "
        "right: 4px;}"
        "QSlider::groove:horizontal {background: rgba(0,0,0,0);}"
        "QSlider::handle:horizontal {background: transparent; margin: 0 -4px;}"
        "QSlider::handle:hover:horizontal {background: rgba(255,255,255,128); "
        "margin: 0 -4px;}"
...
void ClickthroughSlider::resetMasks() {
  if (this->leftToRight)
    this->setMask(QRegion(0, 0, __max(offset, this->value()),
                          this->parentWidget()->height()));
  else
    this->setMask(QRegion(__min(this->width() - offset, this->value()), 0,
                          __max(offset, this->width() - this->value()),
                          this->parentWidget()->height()));
}
