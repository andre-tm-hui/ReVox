...
WaveformViewer::WaveformViewer(QWidget *parent)
    : QWidget{parent}, LoggableObject("Qt-WaveformViewer") {
  smallest = largest = rms = std::vector<float>(this->width(), 0);
  connect(this, SIGNAL(startRepaint()), this, SLOT(slotRepaint()),
          Qt::ConnectionType::QueuedConnection);
  connect(this, SIGNAL(transitionDone()), this, SLOT(cleanup()),
          Qt::ConnectionType::QueuedConnection);
  connect(this, SIGNAL(signalTransition()), this, SLOT(slotTransition()),
          Qt::ConnectionType::QueuedConnection);
}
...
void WaveformViewer::paintEvent(QPaintEvent *event) {
  ...
  for (int x = 0; x < this->width(); x++) {
    ...
    painter.drawLine(
        x,
        (int)((this->height() / 2.f) - (smallest[x] * (this->height() / 2.f))),
        x,
        (int)((this->height() / 2.f) - (largest[x] * (this->height() / 2.f))));
    painter.setPen(rmsLines);
    painter.drawLine(
        x, (int)((this->height() / 2.f) - (rms[x] * (this->height() / 2.f))), x,
        (int)((this->height() / 2.f) + (rms[x] * (this->height() / 2.f))));
  }
}
...
void WaveformViewer::transition(std::string path, int duration, int delay,
                                ...
                                std::vector<float> *rms, WaveformViewer *wv,
  ...
  if (duration > 0) {
    ...
    for (int j = 0; idx < (int)smallest_t.size();
         idx += iInterval, j += iInterval) {
...
void WaveformViewer::startTransition(std::string path, int delay,
                                     int duration) {
  this->path = path;
  QWidget::window()->setEnabled(false);
  std::thread t(transition, path, delay, duration, &smallest, &largest, &rms,
                this, &length, &sampleRate);
  t.detach();
  log(INFO, "Changing waveform");
}
...
QString WaveformViewer::currentTime() {
  ...
  if (seconds < 10) out.insert(out.size() - 1, '0');
  return out;
}
