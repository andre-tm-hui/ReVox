...
FXKeyPicker::FXKeyPicker(std::shared_ptr<FXManager> fm, std::string fx,
                         std::string param, QWidget *parent)
    : FXParam(fm, fx, param, parent), ui(new Ui::FXKeyPicker) {
  ui->setupUi(this);

  menu = new QMenu();
  menu->setStyleSheet(
      "QMenu { background: #202020; color: #D0D0D0; }"
...
void FXKeyPicker::onKeyToggle(int key) {
  ...
  newVal = noteCheckboxes[key]->checkState() == Qt::CheckState::Checked
               ? newVal - diff
               : newVal + diff;
  fm->ApplyFXSettings({{fx, {{param, newVal}}}});
  fm->UpdateSettings(path, newVal);
  log(INFO, labels[key].toStdString() + " toggled");
}
...
void FXKeyPicker::onScaleSet(int scale) {
  ...
  fm->UpdateSettings(path, newVal);
  setValue(newVal);
  log(INFO, labels[scale].toStdString() + " scale set");
}
