...
HotkeyListWidget::HotkeyListWidget(QSize size, std::shared_ptr<BaseManager> bm,
                                   std::string path, QWidget *parent)
    : QWidget{parent},
      LoggableObject("Qt-HotkeyListWidget"),
      size(size),
      bm(bm),
      path(path) {
  this->resize(size);
  ...
  list->setStyleSheet(
      "QListWidget {background-color: transparent; border: 0px; outline: 0;}"
  ...
  addIcon.addFile(":/rc/hotkeyList/addDisabled.png", QSize(13, 13),
                  QIcon::Disabled);
  ...
  remove->setStyleSheet(
      "QToolButton {background-color: #202020; border: 0px; border-right: 1px "
      "solid #181818; border-top-left-radius: 10px;}"
      "QToolButton:hover {background-color: #303030;}");
  QIcon removeIcon = QIcon();
  removeIcon.addFile(":/rc/hotkeyList/removeIdle.png", QSize(13, 13),
                     QIcon::Normal);
  removeIcon.addFile(":/rc/hotkeyList/removeDisabled.png", QSize(13, 13),
                     QIcon::Disabled);
  removeIcon.addFile(":/rc/hotkeyList/removeHover.png", QSize(13, 13),
                     QIcon::Active);
...
void HotkeyListWidget::initializeList() {
  ...
  for (int i = 0; i < hotkeys.size(); i++) addHotkey(false);
  log(INFO, "List populated");
}

void HotkeyListWidget::selectionChanged() {
  if (bm->GetSetting(path + "/" + std::to_string(list->currentRow()) +
                     "/editable")
          .get<bool>())
  ...
  log(INFO, "Row " + std::to_string(list->currentRow()) + " selected");
}

void HotkeyListWidget::addHotkey(bool isNew) {
  HotkeyListItemWidget *item = new HotkeyListItemWidget(
      bm, path + "/" + std::to_string(list->count()), itemHeight, isNew, list);
  ...
  list->scrollToBottom();
  list->resize(list->width(),
               __min(itemHeight * list->count() + 2, size.height()));

  emit hotkeyAdded();
  log(INFO, "New hotkey added");
}
...
void HotkeyListWidget::removeHotkey() {
  ...
  std::string filename =
      bm->GetRootDir() + "samples/" + std::to_string(currentRow) + ".mp3";
  ...
  for (int i = currentRow; i < list->count(); i++) {
    ...
    if (std::filesystem::exists(filename)) {
      std::filesystem::rename(
          filename, bm->GetRootDir() + "samples/" + std::to_string(i) + ".mp3");
    }
    item =
        qobject_cast<HotkeyListItemWidget *>(list->itemWidget(list->item(i)));
  ...
  emit hotkeyRemoved();
  log(INFO, "Hotkey " + std::to_string(currentRow) + " removed");
}
