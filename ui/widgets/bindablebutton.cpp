...
BindableButton::BindableButton(std::shared_ptr<BaseManager> bm,
                               std::string path, bool isNew, QWidget *parent)
    : QPushButton(parent),
      LoggableObject("Qt-BindableButton"),
      path(path),
      bm(bm) {
  this->setCursor(QCursor(Qt::CursorShape::ArrowCursor));
  this->setStyleSheet(
      "QPushButton {margin: 3px; border-radius: 3px; background-color: "
      "#303030; color: #D0D0D0;}"
      "QPushButton:hover {background-color: #404040;}");

  connect(this, SIGNAL(pressed()), this, SLOT(startRebinding()));
  connect(this, SIGNAL(newHotkeySet()), this, SLOT(cleanUp()),
...
void BindableButton::waitForRebind(BindableButton *hkb,
