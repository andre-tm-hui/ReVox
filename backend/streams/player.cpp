...
Player::Player(device inputDevice, device outputDevice, int sampleRate, int framesPerBuffer, std::string dir, std::queue<float> *bufQueue)
...
void Player::Play(int keycode, json settings)
{
    ...
    int startAt = settings["startAt"].get<int>(),
        readRemaining = startAt,
        readN = 1;
...
void Player::Record(int keycode)
{
    if (!recording) {
        recording = true;
